...
- `chrome-mobile-menu` shows `Open menu` at `≤900px`. That button opens `#mobileMenu` (`Navigation`) with the same Home / About / Musings / Library / TIL links plus social icons (LinkedIn, YouTube, X, GitHub, email).
...
- The drawer copies only social `a` links (LinkedIn, YouTube, X, GitHub, email). `#pokeballRelease` remains in the header strip.
